public class Grid
{
   ...
   public boolean canPlaceShip(Ship s)
   {
       ...
       if(s.getDirection() == VERTICAL)
       {
           for(int i = row; i < row + s.getLength(); i++)
           {
               if(!inBounds(i, col) || hasShip(i, col) || col != 0 && col != 9 && (hasShip(i, col + 1) || hasShip(i, col - 1)))
               {
                   return false;
               }
           }
       ...
       else
       {
           for(int i = col; i < col + s.getLength(); i++)
           {
               if(!inBounds(row, i) || hasShip(row, i) || row != 0 && row != 9 && (hasShip(row + 1, i)  || hasShip(row - 1, i)))
               {
                   return false;
               }
           }
